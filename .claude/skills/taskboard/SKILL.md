...
3. DOING    → taskboard.ps1 status <id> doing "Investigando..."
              ↳ Auto-asigna el agente: si estaba "unassigned", se asigna a "claude" por defecto.
              ↳ Para asignarte explícitamente: taskboard.ps1 status <id> doing "msg" claude
...
# Mover + comentar + auto-asignar a este agente (Claude)
.\tools\taskboard\bin\taskboard.ps1 status task-1234567890-0 doing "Empezando análisis" claude

# Comentar sin mover
.\tools\taskboard\bin\taskboard.ps1 comment task-1234567890-0 "Esperando que el PR se mergee..."

# Ver tareas (incluye el documento adjunto si existe)
...
Cuando la IA genera un plan en `planning`, guarda automáticamente un documento Markdown en `docs/`:
- La tarjeta muestra un badge **📄 Doc** en el tablero visual
- El panel de detalle muestra el link "📄 Documento vinculado"
- `taskboard.ps1 get <id>` imprime el path del documento si existe
- El modal de Docs (botón en la barra superior) permite navegar y leer todos los docs

Si ves "📄 doc/..." en el `get`, léelo antes de empezar a trabajar — tiene el plan detallado.
